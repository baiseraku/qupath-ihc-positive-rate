...
## 开跑前必须问用户（不要拿着默认值直接开跑）

三个参数里 **阳性阈值没有客观默认值**，必须由用户拍板。开跑前用 `ask_user_question` 问清楚，
问题照这个写：

> **DAB 阳性阈值定多少？**
> - 有阴性对照片（同批、同抗体、省略一抗）→ 填阴性片 `Cell: DAB OD mean` 的 p99
> - 没有对照片 → 可以先给一个数（如 0.1）出结果，但结论必须标注「无对照，阈值未经校准」
> - 不确定 → 先用 0.1 跑一遍，再用诊断脚本出阈值曲线，拿曲线回来一起定

顺带确认两件事：**腔室**（核标记 `Nucleus` / 胞浆膜标记 `Cell` 或 `Cytoplasm`）、
**组织检测阈值**（默认 200）。用户没意见就用默认值，但要在回复里说明用了什么。

脚本会把阈值来源写进 CSV 的 `thresholdSource` 列：`explicit`（调用方显式指定）或
`default`（没给、用的默认 0.1）。**没问过用户就出现 `default`，说明这一步被跳过了。**
...
阳性率算得出来 ≠ 算得对。**本流程不会因为任何检查不通过而中止——数值照常输出**，
下面两条是给结果打可信度标签用的，不是运行闸门。发现不可靠时，正确的做法是
**把数报出来 + 明确标注不可靠 + 说明原因**，而不是不跑、不报。

跑完先查 `ihc_summary.csv` 的 **`estimatedAngleDeg`** 与 **`reliability`** 列，
再用 `<skill>/scripts/ihc_qc_probe.groovy` 做阈值扫描。

### 标注一：色向量夹角 `estimatedAngleDeg` → `reliability` 列
...
| 夹角 | `reliability` | 判读 |
|---|---|---|
| ≥ 30° | `ok` | 正常，DAB 可以被解卷积分离出来 |
| 20–30° | `suspect_angle` | 偏低，存疑，结合染色形态一起看 |
| **< 20°** | `degenerate_angle` | **退化**。估计器只能把苏木素的轴硬劈成两半，DAB OD 基本是串扰+背景偏移，阳性率不可信 |
| 估计失败 | `angle_unknown` | 读图或估计算法出错，结果按不可信处理 |

夹角退化说明**这批片没有可用的 DAB 显色**，此时调阈值、换腔室、加对照都救不回来，
必须回去查染色（阳性对照 / 抗体浓度 / 抗原修复 / DAB 显色时间）。
但**结论仍然照常产出**，只是每条数字后面都要带上「该片夹角仅 X°，结果不可靠」。
...
## CSV 写入行为（多图批处理，中途读文件也安全）

`ihc_summary.csv` 由每张图的脚本各自读改写一次，因此有三条约定：

- **表头变更时丢弃旧行，不混写**。旧行的列按新表头解析会整体错位——例如旧表头第 14 列是
  `elapsedSec`、新表头第 14 列是 `IOD`，混在一起读就会把耗时当成积分光密度。
  脚本比对表头，不一致时把旧文件备份为 `ihc_summary_legacy_<时间戳>.csv` 再从空重建，
  不丢数据。
- **原子写**：先写 `ihc_summary.csv.tmp` 再 `ATOMIC_MOVE` 改名。直接覆写会让并发读取者
  读到半截文件。
- **行按图名排序**，同一张图重跑覆盖自己的行；文件在多次运行之间保持稳定，可直接 diff。

> 后果：批处理**进行中**读这个文件，只会看到已处理完的图（而不是全部图里夹杂错位的行）。
> 想知道还剩哪些没跑，看 `results/qc/` 里已生成的 QC 图数量。
...
2. 开跑前已就阳性阈值问过用户；CSV 的 `thresholdSource` 列应为 `explicit`，
   出现 `default` 说明跳过了询问这一步
3. 夹角 <20° 的片会有 `[IHC-QC]` 警告、`reliability` 列为 `degenerate_angle` ——
   **照常出数**，但报数时必须一并声明该片结果不可靠
4. **逐张看 QC 图**：绿线是否贴合组织（多块组织、卷绕切片尤其要查）、红点是否落在合理的组织学位置。
