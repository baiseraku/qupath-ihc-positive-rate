...
本流程把这两道判断做成了内置闸门：自动检测扫描背景并校正色向量，同时逐片估计 H–DAB 夹角作为
可信度指标（夹角小于 20° 说明该片没有可用的 DAB 信号，直接报警）。附带的诊断脚本可一次性输出
背景众数、色向量夹角、DAB OD 分布与阈值扫描表，用于判断阳性率是否可用。
...
**第 3 步：判读**。先看 `ihc_summary.csv` 的 `estimatedAngleDeg` 列（应 ≥30°），
再看诊断脚本输出的阈值扫描表是否有平台段。两道闸门都过，阳性率才可报告。
判读细则见 `SKILL.md` 的《结果判读》。
